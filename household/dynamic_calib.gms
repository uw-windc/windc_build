...
* End routine if calibrating the static dataset
$if %invest%=="static" $exit


* -----------------------------------------------------------------------------
* Read in the base WiNDC dataset
* -----------------------------------------------------------------------------
...
* In economics, when theory and the facts diverge, too bad for the facts. Report
* reference values for capital payments and investments.
...
investment(rb(r))..	sum(g,I0_V(r,g)) =e= sum(s, (gr+delta) * kd0(r,s)/(ir+delta));
...
ys0(r,s,g) = round(YS0_V.L(r,s,g),6);
id0(r,g,s) = round(ID0_V.L(r,g,s),6);
dd0(r,g) = round(DD0_V.L(r,g),6);
nd0(r,g) = round(ND0_V.L(r,g),6);
xd0(r,g) = round(XD0_V.L(r,g),6);
xn0(r,g) = round(XN0_V.L(r,g),6);
a0(r,g) = round(A0_V.L(r,g),6);
s0(r,g) = round(S0_V.L(r,g),6);
i0(r,g) = round(I0_V.L(r,g),6);
g0(r,g) = round(G0_V.L(r,g),6);
...
* Include assumed steady-state rates

execute_unload '%gdxdir%dynamic_parameters_%year%.gdx'
    ys0, id0, dd0, nd0, xd0, xn0, a0, s0, i0, g0, ty0, ta0, ir, gr, delta;
