...
EXECUTE 'gams %reldir%%sep%sectordisagg.gms o="%reldir%%sep%temp%sep%lst%sep%sectordisagg.lst" --year=%year% --aggr=%aggr% > %system.nullfile%';
...
EXECUTE 'gams %reldir%%sep%aggregate.gms o="%reldir%%sep%temp%sep%lst%sep%aggregate.lst" --year=%year% --sdisagg=yes --aggr=%aggr% --reldir=%reldir% > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%%wdir%%sep%aggchk.gms --neos=yes solver=kestrel optfile=1 o="%reldir%%sep%temp%sep%lst%sep%aggchk.lst" --neosserver=%neosserver% --kestrel_mcp=%kestrel_mcp% --kestrel_nlp=%kestrel_nlp% --kestrel_lp=%kestrel_qcp% --year=%year% --aggr=%aggr% > %system.nullfile%';
myerrorlevel = errorlevel;
ABORT$(myerrorlevel <> 0) "ERROR: aggchk.gms did not complete with error code '0'";

$ELSE

EXECUTE 'gams %reldir%%sep%aggchk.gms o="%reldir%%sep%temp%sep%lst%sep%aggchk.lst" --year=%year% --aggr=%aggr% --reldir=%reldir% > %system.nullfile%';
myerrorlevel = errorlevel;
ABORT$(myerrorlevel <> 0) "ERROR: aggchk.gms did not complete with error code '0'";
