...
*------------------------------------------------------------------------------
$IF NOT DEXIST "temp" $CALL mkdir "temp"
$IF NOT DEXIST "built_datasets" $CALL mkdir "built_datasets"
...
EXECUTE 'gams %reldir%%sep%partitionbea.gms o="%reldir%%sep%temp%sep%lst%sep%partitionbea.lst" > %system.nullfile%';
...
EXECUTE 'gams %reldir%%sep%calibrate.gms qcp=cplex nlp=ipopt --year=%year% --matbal=ls o="%reldir%%sep%temp%sep%lst%sep%calibrate.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
ABORT$(myerrorlevel <> 0) "ERROR: calibrate.gms did not complete with error code '0'";
...
EXECUTE 'gams %reldir%%sep%gspshare.gms o="%reldir%%sep%temp%sep%lst%sep%gspshare.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%pceshare.gms o="%reldir%%sep%temp%sep%lst%sep%pceshare.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%sgfshare.gms o="%reldir%%sep%temp%sep%lst%sep%sgfshare.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%cfsshare.gms o="%reldir%%sep%temp%sep%lst%sep%cfsshare.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%usatradeshare.gms o="%reldir%%sep%temp%sep%lst%sep%usatradeshr.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
...
EXECUTE 'gams %reldir%%sep%statedisagg.gms --year=%year% o="%reldir%%sep%temp%sep%lst%sep%statedisagg.lst" > %system.nullfile%';
myerrorlevel = errorlevel;
ABORT$(myerrorlevel <> 0) "ERROR: statedisagg.gms did not complete with error code '0'";
...
EXECUTE 'rm build.lst';
$ENDIF

$IFTHENI %system.filesys% == MSNT
EXECUTE 'del build.lst';
$ENDIF
