...
*	Could implement some sort of spline fit for years prior to 2007 
*	and after 2017, but for now we use the simplest form:
...
set vabas(*) /v001,v003,t00otop, t00osub/;
...
market_s(g(ru_s),yrs,"final")   = sum(fd(cu_s),use_s(yrs,ru_s,cu_s)) - use_s(yrs,ru_s,"F040");
...
equations	objdef, profitbal, marketbal;
...
profitbal(d)..
	sum(unz(ru_d(gg),cu_d(d)),   USE_(ru_d,cu_d)) + sum(unz(ru_d(vabas),cu_d(d)),USE_(ru_d,cu_d)) =e= 

		sum(snz(rs_d(gg),cs_d(d)), SUPPLY_(rs_d,cs_d));

marketbal(gg)..
	SUM(rs_d(gg),
	  sum(snz(rs_d,cs_d(d)), SUPPLY_(rs_d,cs_d)) + 
		SUPPLY_(rs_d,"MCIF") + SUPPLY_(rs_d,"MADJ") + 
		SUPPLY_(rs_d,"trade") + SUPPLY_(rs_d,"trans") +
		SUPPLY_(rs_d,"mdty")  + SUPPLY_(rs_d,"top")   + SUPPLY_(rs_d,"sub")) =e=
	sum(ru_d(gg), 
	  sum(unz(ru_d,cu_d(d)),  USE_(ru_d,cu_d)) + 
	  sum(unz(ru_d,cu_d(fd)), USE_(ru_d,cu_d)));
	
model lsqcalib /objdef, profitbal, marketbal/;
...
execute_unload 'data\projected.gdx', use, supply;

set	profit_col /interm, (set.vabas), revenue, balance/	
	market_col /produc, import, margins, taxes, interm, export, final, balance /;
...
profit(d,yrs,profit_col)$(not round(profit(d,yrs,"balance"),3)) = 0;
display profit;

market(d,yrs,market_col)$(not round(market(d,yrs,"balance"),3)) = 0;
display market;
