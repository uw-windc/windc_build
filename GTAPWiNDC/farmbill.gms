...
set	fbp(i)	Food - beverage - tobacco products /fbp/;

parameter	tc(i,r,s,h)	Consumption tax rate;
tc(i,r,s,h) = 0;
...
$prod:C(r,s,h)$c_(r,s,h)  s:1
	...
	i:PZ(i,r,s)	q:cd0(i,r,s,h)	a:GOVT(r)  t:tc(i,r,s,h)
...
parameter	expend(*,i,h)	Household consumption expenditures;
expend("$",agrfoo(i),h) = sum(s,cd0(i,"usa",s,h));
expend("%",agrfoo(i),h)$sum((i.local,s),cd0(i,"usa",s,h))
			= 100 * sum(s,cd0(i,"usa",s,h)) / 
				sum((i.local,s),cd0(i,"usa",s,h));
option expend:1:2:1;
display expend;
