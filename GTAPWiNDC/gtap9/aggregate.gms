...
*.$if not set target $set target g20_43
$if not set target $set target wb62_14
...
*.	nco2process_(pol,ii,jj,rr)	'IO-based process emissions, mmt',
...
*.nco2process_(pol,ii,jj,rr) = sum((mapi(i,ii),mapj(j,jj),mapr(r,rr)), nco2process(pol,i,j,r));
...
execute_unload "%system.fp%/gtap9/%yr%/%output%.gdx", 
	gg=g, rr=r, ff=f, ii=i, 
...
*.	nco2emit_=nco2emit, nco2process_=nco2process, landuse_=landuse,
	eta_=eta, aues_=aues, incp_=incp, subp_=subp, metadata;
