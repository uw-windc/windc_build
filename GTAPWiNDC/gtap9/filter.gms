...
nz("vst","gtap9") = card(vst);
nz("vtwr","gtap9") = card(vtwr);
nz("vxmd","gtap9") = card(vxmd);
nz("vdfm","gtap9") = card(vdfm);
nz("vifm","gtap9") = card(vifm);
nz("vfm","gtap9") = card(vfm);
...
nz("vst","abstol") = card(vst) - nz("vst","gtap9");
nz("vtwr","abstol") = card(vtwr) - nz("vtwr","gtap9");
nz("vxmd","abstol") = card(vxmd) - nz("vxmd","gtap9");
nz("vdfm","abstol") = card(vdfm) - nz("vdfm","gtap9");
nz("vifm","abstol") = card(vifm) - nz("vifm","gtap9");
nz("vfm","abstol") = card(vfm) - nz("vfm","gtap9");
...
nz("vst","reltol")  = card(vst)  - nz("vst","gtap9")  - nz("vst","abstol");
nz("vtwr","reltol") = card(vtwr) - nz("vtwr","gtap9") - nz("vtwr","abstol");
nz("vxmd","reltol") = card(vxmd) - nz("vxmd","gtap9") - nz("vxmd","abstol");
nz("vdfm","reltol") = card(vdfm) - nz("vdfm","gtap9") - nz("vdfm","abstol");
nz("vifm","reltol") = card(vifm) - nz("vifm","gtap9") - nz("vifm","abstol");
nz("vfm","reltol")  = card(vfm)  - nz("vfm","gtap9")  - nz("vfm","abstol");
...
nz(param,"%")  = round(100 * nz(param,"card")/nz(param,"gtap9"));
option nz:0;
display nz;
...
execute_unload '%system.fp%/gtap9/%yr%/gtapingams_%reltol%.gdx',f,g,i,r,pol,
	vfm,vdfm,vifm,vxmd,vst,vtwr,
	rto,rtf,rtfd,rtfi,rtxs,rtms,
	subp, incp, etaf, esubva, esubdm, eta, aues, 
	evd, evi,
*.evt, nco2emit, nco2process, landuse, 
	eco2d, eco2i, pop, metadata;
