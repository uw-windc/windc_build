...
set acct /vom  "gross output"
          rto  "output tax payment"
	  vafm "intermediate input"
	  vfm  "primary factor input"
	  rtf  "direct taxes on factors"
	  /,
    metric /B_USD,share/
    ix /x,set.i/
    fx /x,set.f/
...
Parameter
	CB_rpt(s,acct,metric,*,*);

Loop((cb,s)$vom(cb,"usa",s),
CB_rpt(s,"vom","B_USD","x","x") = vom(cb,"usa",s);
CB_rpt(s,"rto","B_USD","x","x") = vom(cb,"usa",s)*rto(cb,"usa");
CB_rpt(s,"rto","share","x","x") = vom(cb,"usa",s)*rto(cb,"usa")/vom(cb,"usa",s);
CB_rpt(s,"vafm","B_USD",i,"x")  = vafm(i,cb,"usa",s);
CB_rpt(s,"vafm","share",i,"x")  = vafm(i,cb,"usa",s)/vom(cb,"usa",s);
CB_rpt(s,"vfm","B_USD","x",f)   = vfm(f,cb,"usa",s);
CB_rpt(s,"vfm","share","x",f)   = vfm(f,cb,"usa",s)/vom(cb,"usa",s);
CB_rpt(s,"rtf","B_USD","x",f)   = vfm(f,cb,"usa",s)*rtf(f,cb,"usa");
CB_rpt(s,"rtf","share","x",f)   = vfm(f,cb,"usa",s)*rtf(f,cb,"usa")/vom(cb,"usa",s);
);

execute_unload 'cbrpt.gdx', cb_rpt;

execute 'gdxxrw i=cbrpt.gdx o=cbrpt.xlsx par=cb_rpt rng=cbrpt!a2 cdim=0';
