...
parameter	chkmarket	Cross check on market clearance;
$ondotl
chkmarket(i_P,"Y") = sum(j_Y,   a(P,Y) * Y(j_Y));
chkmarket(i_P,"M") = sum(j_M, a(P,M) * M(j_M));
chkmarket(i_P,"YT") = sum(j_YT,     a(P,YT) * YT(j_YT));
chkmarket(i_P,"D") =  sum(h_RA,     d(P,RA));
chkmarket(i_P,"E") =  sum(h_RA,     e(RA,P));
chkmarket(i_P,"chk") = chkmarket(i_P,"Y") + chkmarket(i_P,"M")
		+ chkmarket(i_P,"YT") - chkmarket(i_P,"D") + chkmarket(i_P,"E");
option chkmarket:3:2:1;
display chkmarket;
...
profitchk(j_M(i,r),"PM") = sum(i_PM(i,r), PM(i,r) * a(PM,M));
profitchk(j_M(i,r),"P") =  sum(i_P(i,rr),  P(i,rr)  * a(P,M));
profitchk(j_M(i,r),"PT") =  sum(i_PT(j),   PT(j)   * a(PT,M));
profitchk(j_M(i,r),"TAU") = + sum(h_RA,      tau(M,RA));
profitchk(j_M(i,r),"CHK") = profitchk(j_M,"PM") 
		+ profitchk(j_M,"P") 
		+ profitchk(j_M,"PT")
		- profitchk(j_M,"TAU") ;

option profitchk:3:2:1;
display profitchk; 

$exit

$ondotl
profitchk(j_Y,"P") = sum(i_P,  P(i_P)   * a(P,Y));
profitchk(j_Y,"PM") = sum(i_PM, PM(i_PM) * a(PM,Y));
profitchk(j_Y,"PS") = sum(i_PS, PS(i_PS) * a(PS,Y));
profitchk(j_Y,"PF") = sum(i_PF, PF(i_PF) * a(PF,Y));
profitchk(j_Y,"RA") = sum(h_RA, tau(Y,RA));
profitchk(j_Y,"chk") = profitchk(j_Y,"P") +
	profitchk(j_Y,"PM") +
	profitchk(j_Y,"PS") + 
	profitchk(j_Y,"PF") - profitchk(j_Y,"RA");
option profitchk:3:2:1;
display profitchk; 
