...
parameter
    sedsenergy_(rr,pq,e,ed,yr)		"SEDS energy data for IO integration",
    ...
    seds_units_(source,sector,rr,yr,*)	"Complete EIA SEDS data, with units as domain";

alias (yr,yrh);

*	Price data is aggregated using weighted average

sedsenergy_(rr,'q',e,ed,yrh) = sum(rmap(rr,r), sedsenergy(r,'q',e,ed,yrh));
sedsenergy_(rr,'p',e,ed,yrh)$sum(rmap(rr,r), sedsenergy(r,'q',e,ed,yrh)) =
    sum(rmap(rr,r), sedsenergy(r,'p',e,ed,yrh)*sedsenergy(r,'q',e,ed,yrh)) / sum(rmap(rr,r), sedsenergy(r,'q',e,ed,yrh));
...
seds_units_(source,sector,rr,yrh,u) = sum(rmap(rr,r), seds_units(source,sector,r,yrh,u));
seds_units_(source,sector,rr,yrh,ru) = sum(rmap(rr,r), seds_units(source,sector,r,yrh,ru)) / sum(rmap(rr,r), 1);
