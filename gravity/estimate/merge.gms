...
* $prod:B(i,r,rr)$b_(i,r,rr)  s:esubm(i)  rr.tl:0
* 	o:PB(i,r,rr)	q:vbm(i,r,rr)
* 	i:P(i,rr)	q:vxmd(i,rr,r)	p:pvxmd(i,rr,r) rr.tl:
* +		a:GOVT(rr) t:(-rtxs(i,rr,r))	
* +		a:GOVT(r)  t:(rtms(i,rr,r)*(1-rtxs(i,rr,r)))
* 	i:PT(j)#(rr)	q:vtwr_(j,i,rr,r) p:pvtwr(i,rr,r) rr.tl: 
...
*	Then read the gravity estimates of domestic and international trade:
...
$if not set ds $set ds b

$call gdxmerge datasets\%ds%\*.gdx
$gdxin 'merged.gdx'
$load m0 x0 y0 z0 nref mref xref

set	itrd(i)		Sectors with domestic trade data
	strd(s)		States with domestic trade;

option itrd<nref, strd<nref;

set	nstrd(s); nstrd(s) = not strd(s); option nstrd:0:0:1; display nstrd;

alias (strd,sstrd);
bd0(itrd(i),"usa",strd(s),sstrd(ss)) = nref(i,s,ss);
md0(itrd(i),"usa",strd(s))    = sum(r,mref(i,r,s));
xs0(itrd(i),"usa",strd(s))    = sum(r,xref(i,s,r));
n_(itrd(i),mkt,"usa")$(not card(nstrd))  = no;
pn_(itrd(i),mkt,"usa")$(not card(nstrd)) = no;
yd0(itrd(i),"usa",strd(s)) = bd0(i,"usa",s,s);
bd0(itrd(i),"usa",strd(s),s) = 0;
nd0(itrd(i),mkt,"usa",strd(s)) = 0;
