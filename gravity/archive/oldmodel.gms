...
$prod:Z(s)$z_(s)  s:esubdm mm:(2*esubdm)  r.tl(mm):0 dn:(2*esubdm)  nn(dn):(4*esubdm)
	o:PZ(s)		q:(lamda(s)*z0(s))
	i:PY(ss)	q:(tau_d(ss,s)*bdref(ss,s))	p:(1/tau_d(ss,s))  dn:$sameas(s,ss) nn:$(not sameas(s,ss))
	i:PI(r,pd)	q:(tau_m(pd,s)*mref(r,pd,s))	p:(1/tau_m(pd,s))  r.tl:
...
$constraint:BM(r)$mb0(r)
	sum(pd,m0(r,pd)*PI(r,pd)) =e= PFX*mb0(r);
